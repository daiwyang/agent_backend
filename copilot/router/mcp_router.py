    ...
    try:
        ...
        if success:
            ...
            try:
                ...
                return {"success": True, "message": f"成功连接到服务器: {server_config.name}", "updated_sessions": len(updated_sessions)}
            except Exception as reload_error:
                logger.error(f"Failed to update agents after server connection: {str(reload_error)}")
                # 连接成功，但Agent更新失败
                return {"success": True, "message": f"服务器连接成功: {server_config.name}，但部分Agent更新失败", "warning": str(reload_error)}
        else:
...
async def get_agent_mcp_servers(session_id: str, current_user: dict = Depends(get_current_user_from_state)):
    ...
    try:
        ...
        agent_servers = [server for server in all_servers if server["id"] in server_ids]

        return {"success": True, "session_id": session_id, "server_ids": list(server_ids), "servers": agent_servers, "server_count": len(server_ids)}
    except Exception as e:
...
async def set_agent_mcp_servers(session_id: str, config: AgentMCPConfig, current_user: dict = Depends(get_current_user_from_state)):
    ...
    try:
        ...
        if invalid_ids:
            raise HTTPException(status_code=400, detail=f"无效的服务器ID: {list(invalid_ids)}")
