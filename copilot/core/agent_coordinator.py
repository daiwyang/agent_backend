...
class AgentCoordinator:
    ...
    async def process_user_input(
        ...
        try:
            ...
            if self.enable_thinking_mode:
                async for chunk in self._process_with_thinking(
                    user_input, session_id, thread_id, images, enable_tools, context
                ):
                    yield chunk
            else:
                # 直接执行模式
                async for chunk in self._process_direct_execution(
                    user_input, session_id, thread_id, images, enable_tools
                ):
                    yield chunk
    ...
    async def _process_with_thinking(
        ...
        try:
            ...
            # 3. 思考Agent分析
            thinking_result = await self.thinking_agent.think(
    ...
    async def _get_conversation_history(self, session_id: str) -> List[Dict]:
        ...
        try:
            ...
            if hasattr(self.execution_agent, 'chat_history_manager'):
                history_messages = await self.execution_agent.chat_history_manager.get_session_messages(
                    session_id=session_id, 
                    limit=10
                )
