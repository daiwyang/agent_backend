...
class ChatHistoryManager:
    ...
    async def save_session(self, session_id: str, user_id: str, window_id: str, thread_id: str, context: Dict[str, Any] = None):
        ...
        try:
            async with MongoClient() as mongo:
                ...
                await mongo.insert_one(self.sessions_collection, session_doc)
    ...
    async def get_session_messages(self, session_id: str, limit: int = 100, offset: int = 0) -> List[ChatHistoryMessage]:
        ...
        try:
            async with MongoClient() as mongo:
                messages = await mongo.find_many(
        ...
        try:
            async with MongoClient() as mongo:
                sessions = await mongo.find_many(
                    ...
                    sort=[("last_activity", -1)],  # 按最后活动时间降序
                    limit=limit
                )
    ...
    async def delete_session(self, session_id: str, hard_delete: bool = False):
        ...
        try:
            async with MongoClient() as mongo:
                if hard_delete:
                    # 物理删除
                    await mongo.delete_many(self.messages_collection, {"session_id": session_id})
                    await mongo.delete_one(self.sessions_collection, {"session_id": session_id})
            ...
            async with MongoClient() as mongo:
                ...
                messages = await mongo.find_many(
                    self.messages_collection,
                    {
                        ...
                        "content": {"$regex": query, "$options": "i"}  # 不区分大小写搜索
                    },
                    sort=[("timestamp", -1)],
                    limit=limit
                )
    ...
    async def get_stats(self, user_id: str = None) -> Dict[str, Any]:
        ...
        try:
            async with MongoClient() as mongo:
                ...
                session_count = await mongo.count_documents(self.sessions_collection, filter_dict)
                ...
                message_count = await mongo.count_documents(self.messages_collection, message_filter)
